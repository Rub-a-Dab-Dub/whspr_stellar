...
use crate::types::{ActionType, RateLimitConfig, DailyStats};
...
pub fn check_can_act(env: &Env, user: &Address, action: ActionType) {
    ...
    if env.storage().instance().has(&DataKey::AdminOverride(user.clone())) {
        if env.storage().instance().get(&DataKey::AdminOverride(user.clone())).unwrap_or(false) {
            return; // User is exempt from limits
        }
    }

    // 2. Load Configuration
    let config: RateLimitConfig = env.storage().instance().get(&DataKey::RateLimitConfig)
        .expect("RateLimitConfig not initialized");

    // 3. Load User Reputation (Default to 0)
    let reputation: u32 = env.storage().instance().get(&DataKey::UserReputation(user.clone())).unwrap_or(0);
    ...
    let mut daily_stats: DailyStats = env.storage().instance().get(&daily_stats_key).unwrap_or(DailyStats::default());
...
pub fn record_action(env: &Env, user: &Address, action: ActionType) {
    ...
    let last_action_key = DataKey::UserLastAction(user.clone(), action);
    env.storage().instance().set(&last_action_key, &current_time);
