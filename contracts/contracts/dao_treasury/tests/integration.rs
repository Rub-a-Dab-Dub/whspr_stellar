...
fn test_full_dao_flow_integration() {
    ...
    let proposal_id =
        client.create_proposal(&group_id, &recipient, &600, &Symbol::new(&env, "ops"));
...
fn test_expiry_blocks_execution_integration() {
    ...
    let proposal_id =
        client.create_proposal(&group_id, &recipient, &100, &Symbol::new(&env, "exp"));
    ...
    assert!(matches!(
        res,
        Err(Ok(dao_treasury::ContractError::ProposalExpired))
    ));
}
