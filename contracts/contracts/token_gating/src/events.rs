...
pub fn emit_gate_set(env: &Env, group_id: &GroupId, token: &Address, min_balance: i128, admin: &Address) {
    env.events().publish(
...
pub fn emit_gate_removed(env: &Env, group_id: &GroupId, removed_by: &Address) {
    env.events().publish(
        (Symbol::new(env, "gate_removed"), group_id),
        removed_by,
    );
}

/// Emitted when a user is denied access due to insufficient token balance
pub fn emit_access_denied(env: &Env, group_id: &GroupId, user: &Address, required: i128, actual: i128) {
    env.events().publish(
