...
import { WithdrawalRequest, WithdrawalStatus, ChainType } from './entities/withdrawal-request.entity';
...
const mockAdmin = { id: 'admin-1', username: 'admin_bob', ipAddress: '127.0.0.1' };
...
describe('WithdrawalsService', () => {
  ...
  beforeEach(async () => {
    ...
    auditLogService = { log: jest.fn().mockResolvedValue({}), findByWithdrawalId: jest.fn() };
    notificationService = { notifyUser: jest.fn().mockResolvedValue(undefined) };
    blockchainQueueService = { enqueue: jest.fn().mockResolvedValue({ jobId: 'job-1' }) };
    riskScoringService = {
      assessRisk: jest.fn().mockResolvedValue({ score: 20, isNewAddress: false, flags: [] }),
    };
  ...
  describe('createRequest', () => {
    it('should create a pending request for amounts above threshold', async () => {
      const dto = {
        ...
        walletAddress: '0xAbc', amount: 500, chain: ChainType.ETH,
      };
    ...
    it('should auto-approve requests below threshold with low risk', async () => {
      const dto = {
        userId: 'user-1', username: 'alice',
        walletAddress: '0xAbc', amount: 50, chain: ChainType.ETH,
      };
      const created = mockRequest({ amount: 50 });
      const queued = mockRequest({ amount: 50, status: WithdrawalStatus.QUEUED, autoApproved: true });
      withdrawalRepo.create.mockReturnValue(created);
      withdrawalRepo.save.mockResolvedValue(created);
    ...
    it('should NOT auto-approve if risk score is >= 30 even below threshold', async () => {
      riskScoringService.assessRisk.mockResolvedValue({ score: 50, isNewAddress: true, flags: ['NEW_WALLET_ADDRESS'] });
      const dto = {
        userId: 'user-1', username: 'alice',
        walletAddress: '0xNew', amount: 50, chain: ChainType.ETH,
      };
  ...
  describe('approveRequest', () => {
    it('should approve a pending request', async () => {
      ...
      const approved = mockRequest({ status: WithdrawalStatus.QUEUED, reviewedBy: mockAdmin.id });
      ...
      expect(auditLogService.log).toHaveBeenCalledWith(
        expect.objectContaining({ action: AuditAction.APPROVED, adminId: mockAdmin.id }),
      );
      expect(notificationService.notifyUser).toHaveBeenCalledWith(
    ...
    it('should throw NotFoundException for unknown request', async () => {
      ...
      await expect(service.approveRequest('bad-id', mockAdmin)).rejects.toThrow(NotFoundException);
    });

    it('should throw ConflictException if request is not pending', async () => {
      withdrawalRepo.findOne.mockResolvedValue(mockRequest({ status: WithdrawalStatus.REJECTED }));
      await expect(service.approveRequest('req-uuid-1', mockAdmin)).rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('rejectRequest', () => {
    it('should reject a pending request with reason', async () => {
      const pending = mockRequest();
      const rejected = mockRequest({ status: WithdrawalStatus.REJECTED, rejectionReason: 'Suspicious' });
      withdrawalRepo.findOne
        .mockResolvedValueOnce(pending)
      ...
      expect(notificationService.notifyUser).toHaveBeenCalledWith(
        expect.objectContaining({ type: 'WITHDRAWAL_REJECTED', reason: 'Suspicious' }),
      );
    });
  });
});
