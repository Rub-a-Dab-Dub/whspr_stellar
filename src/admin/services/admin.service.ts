...
export class AdminService {
  ...
  private async safeLogDataAccess(
    ...
    try {
      ...
      this.logger.warn(`Failed to log data access: ${message}`);
    }
  }
  ...
  async banUser(
    ...
    void this.eventEmitter.emit(ADMIN_STREAM_EVENTS.USER_BANNED, {
      type: 'user.banned',
  ...
  async bulkAction(
    ...
    for (const user of users) {
      try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        errors.push(`Failed to process user ${user.email}: ${message}`);
        failed++;
      }
    }
  ...
  async adminResetPassword(
    ...
    try {
      void this.eventEmitter.emit('user.password.reset.admin', {
        userId,
        email: user.email,
        resetToken,
        adminId,
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.logger.warn(
        `Password-reset event emit failed for user ${userId}: ${message}`,
      );
    }
  ...
  async closeRoom(
    ...
    await this.logAudit(
      adminId,
      AuditAction.ROOM_CLOSED,
      null,
      closeRoomDto?.reason || 'Room closed by admin',
      { roomId, reason: closeRoomDto?.reason || null },
      req,
      AuditSeverity.MEDIUM,
      'room',
      roomId,
    ...
    return { success: true, roomId, status: 'closed' };
  }
  ...
  async deleteRoom(
    ...
    await this.logAudit(
      ...
      null,
      deleteRoomDto?.reason || 'Room deleted by admin',
      { roomId, reason: deleteRoomDto?.reason || null },
      req,
      AuditSeverity.HIGH,
      'room',
      roomId,
    ...
    return { success: true, roomId, status: 'deleted' };
  }
  ...
  async restoreRoom(
    ...
    await this.logAudit(
      adminId,
      AuditAction.ROOM_RESTORED,
      null,
      restoreRoomDto?.reason || 'Room restored by admin',
      { roomId, reason: restoreRoomDto?.reason || null },
      req,
      AuditSeverity.MEDIUM,
      'room',
      roomId,
  ...
  async adjustUserXp(
    ...
    const nextXp = previousXp + dto.delta;
    if (nextXp < 0) {
      throw new BadRequestException('XP cannot go below 0');
    }

    user.currentXp = nextXp;
    user.level = Math.max(1, Math.floor(user.currentXp / 1000) + 1);
    await this.userRepository.save(user);

    // Backward-compatible audit API used in some legacy tests.
    await this.auditLogService.log({
      adminId,
      action: AuditAction.USER_XP_ADJUSTED,
      resourceType: 'USER',
      resourceId: userId,
      details: dto.reason || 'Adjusted user XP',
      changes: {
        previousXp,
    ...
    await this.logAudit(
      adminId,
      AuditAction.USER_XP_ADJUSTED,
      userId,
      dto.reason || 'Adjusted user XP',
      { delta: dto.delta, reason: dto.reason || null },
      req,
      AuditSeverity.MEDIUM,
      'user',
      userId,
    ...
    return {
      ...
      oldLevel: previousLevel,
      newLevel: user.level,
      levelChanged: previousLevel !== user.level,
    };
  }
