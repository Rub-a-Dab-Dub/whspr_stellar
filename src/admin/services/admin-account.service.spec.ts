...
describe('AdminAccountService', () => {
    ...
    describe('listAdmins()', () => {
        it('should return all admin accounts as summaries', async () => {
            const admins = [
                ...
                makeAdmin({ id: 'a2', email: 'a2@example.com', role: UserRole.SUPER_ADMIN }),
            ];
            ...
            expect(result[0]).toMatchObject({ id: 'a1', role: UserRole.ADMIN, isDeactivated: false });
            expect(result[1]).toMatchObject({ id: 'a2', role: UserRole.SUPER_ADMIN, isDeactivated: false });
    ...
    describe('inviteAdmin()', () => {
        ...
        it('should throw ConflictException if email already registered', async () => {
            mockUserRepository.findOne.mockResolvedValue(makeAdmin({ email: 'taken@example.com' }));
    ...
    describe('changeRole()', () => {
        ...
        it('should throw NotFoundException when target does not exist', async () => {
            ...
            await expect(
                service.changeRole('ghost-id', UserRole.MODERATOR, 'reason', 'actor-id'),
        ...
        it('should update role and audit-log on success', async () => {
            ...
            mockUserRepository.save.mockResolvedValue({ ...target, role: UserRole.MODERATOR });
    ...
    describe('deactivateAdmin()', () => {
        ...
        it('should throw BadRequestException when already deactivated', async () => {
            mockUserRepository.findOne.mockResolvedValue(makeAdmin({ isBanned: true }));
        ...
        it('should deactivate account, revoke Redis session, and audit-log', async () => {
            ...
            const result = await service.deactivateAdmin('target-id', 'Security violation', 'actor-id');

            expect(result.message).toContain('deactivated');
            expect(mockRedisService.del).toHaveBeenCalledWith('admin:refresh:target-id');
            expect(mockAuditLogService.createAuditLog).toHaveBeenCalledTimes(1);
        });
    });
    ...
    describe('reactivateAdmin()', () => {
        ...
        it('should throw BadRequestException when account is not deactivated', async () => {
            mockUserRepository.findOne.mockResolvedValue(makeAdmin({ isBanned: false }));
