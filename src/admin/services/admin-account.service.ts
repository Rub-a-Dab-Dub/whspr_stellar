...
export class AdminAccountService {
  ...
  async inviteAdmin(
    ...
    try {
      ...
      this.logger.warn(
        `Failed to send admin invite email to ${email}: ${err.message}`,
      );
    }
  ...
  async deactivateAdmin(
    ...
    return {
      message: `Admin account ${target.email} has been deactivated and all sessions revoked`,
    };
  }
  ...
  private async safeAuditLog(
    ...
    try {
      ...
      this.logger.warn(
        `Failed to write admin account audit log: ${err.message}`,
      );
    }
  }
}
