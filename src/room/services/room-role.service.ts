...
} from '../entities/room-emergency-pause.entity';
...
/* Event constant consumed by GroupKeyEventListener */
...
export class RoomRoleService {
  ...
  async setRoomRole(
    ...
    if (
      ...
      throw new ForbiddenException('Only admins can change other admin roles');
    }
  ...
  async banUser(
    ...
    // Emit event for group key rotation
    this.eventEmitter?.emit(ROOM_MEMBER_BANNED, { roomId, userId });
