...
/* Event constants consumed by GroupKeyEventListener */
...
export class RoomMemberService {
  ...
  async joinRoom(
    ...
    if (queryRunner) {
      ...
      try {
        ...
        this.eventEmitter?.emit(ROOM_MEMBER_JOINED, { roomId, userId });
        return saved;
      } catch (error) {
  ...
  async leaveRoom(userId: string, roomId: string): Promise<void> {
    ...
    await this.invalidateMemberCache(roomId, userId);
    this.eventEmitter?.emit(ROOM_MEMBER_LEFT, { roomId, userId });
  }
  ...
  async kickMember(
    ...
    await this.roomMemberRepository.save(member);
    await this.invalidateMemberCache(roomId, userId);
    this.eventEmitter?.emit(ROOM_MEMBER_KICKED, { roomId, userId });
  }
